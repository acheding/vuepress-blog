...
```python
def __init__(self):
	self.url = 'http://query.china.com.cn/news/queryFn?pagetag=1&noFields=channel&nokws=2&fetch=title%2Curl&pageSize=1000&index=cn%2Cmarket_index'
...
def get_html(self):
    ...
    if not os.path.exists(self.first_path): # 不存在一级目录就新建
    ...
    for i, word in enumerate(self.key_words[word_index:]):
        ...
        if not os.path.exists(self.second_path): # 不存在二级目录就新建
            os.makedirs(self.second_path)
        while True:
            kw = word
            _ = int(time() * 1000) # 获取当前时间戳
            total_url = self.url + f'&startPage={startPage}&kw={kw}&_={_}'
            print(total_url)
            ...
            if res.status_code == 200:
                ...
                sleep(3)
    if word_index == len(self.key_words):
        over = True
```
...
def save_file(self, list):
    global news_index
    currentPage = 1000 * (startPage - 1) + news_index # 计算当前条数
    for i, item in enumerate(list):
        if i < news_index: # 用于中断重启时快速略过
            continue
        title = re.sub(r'<[^>]+>|[\/:*?"<>|]|\n', '', item['title']) # 去除标题中的特殊符号
        url = item['url']
        path = os.path.join(self.second_path, f"{title}")
        try:
            self.url_to_text_and_pdf(url, path)
            ...
            with open(os.path.join(self.second_path, 'error.txt'), 'a', encoding='utf-8') as file:
                file.write(
                    ...
                    f"newsIndex: {news_index}, Title: {title}, URL: {url}\n")
        finally:
            news_index += 1
            print(f"关键词: {self.key_words[word_index]}, {startPage}/{pageCount}页, {currentPage}/{recordCount}条, {title}, {url}")
    news_index = 0
```
...
def url_to_text_and_pdf(self, url, path):
    ...
    try:
        ...
        content += time_tag.text + source_tag.text + '\n' # 添加时间和来源的字符串并换行
        article_tag = self.get_max_tag_parent(soup) # 调用下方第1个方法，找到正文的标签
        vedio_tag = article_tag.find(id="videoarea") # 这里发现部分网站预留了一个“display: none”的视频块，转成PDF时会出现空白，如果其中没有视频就将它移除
        if vedio_tag and not vedio_tag.contents:
            vedio_tag.decompose()
        content += self.get_text(article_tag)  # 调用下方第2个方法，添加正文内容
        with open(f"{path}.txt", "w", encoding="utf-8") as file: # 保存TXT
            file.write(content)
    except Exception as e:
        raise Exception(f"保存TXT异常: {e}")
    try:
        content_prettify = title_tag.prettify() + \
                           time_tag.prettify() + source_tag.prettify() # 将标题、时间、正文转成标准HTML字符串并合并
        title_tag.decompose() # 部分网站标题、时间、来源包含在正文标签中，所以移除掉这三者后再合并正文
        time_tag.decompose()
        source_tag.decompose()
        content_prettify += article_tag.prettify()
...
def get_max_tag_parent(self, soup):
    max_length = 0
    max_tag = None
    white_tag = ["script", "style", "meta", "link", "head", "title", "html", "body", "img"] # 过滤的白名单
...
def get_text(self, tag):
    ...
    for child in tag.contents:
        if isinstance(child, str):
            result += child
        elif child.name == 'img' or child.name == 'source':
            result += child['src']
        else:
